...
def generate_spatial_adj_2d(adata, rad_cutoff=None, k_cutoff=None, model='Radius', spatial_key="spatial"):

    assert model in ['Radius', 'KNN'], "model 必须是 'Radius' 或 'KNN'"
    ...
    if model == 'Radius':
        if rad_cutoff is None:
            raise ValueError("当 model='Radius' 时，必须提供 rad_cutoff")


        nbrs = NearestNeighbors(radius=rad_cutoff).fit(coor)
        indices = nbrs.radius_neighbors(coor, return_distance=False)

        for i, neighbors in enumerate(indices):

            neighbors = neighbors[neighbors != i]
            row_list.extend([i] * len(neighbors))
            col_list.extend(neighbors)

    elif model == 'KNN':
        if k_cutoff is None:
            raise ValueError("当 model='KNN' 时，必须提供 k_cutoff")


        nbrs = NearestNeighbors(n_neighbors=k_cutoff + 1, algorithm='ball_tree').fit(coor)
    ...
    adj = sp.coo_matrix((np.ones(len(row_list)), (row_list, col_list)), shape=(adata.n_obs, adata.n_obs))


    adj.setdiag(1)

    return adj
def generate_radius_adj(adata, radius=50, spatial_key="spatial"):
    """STAligner 对于这个数据集默认使用 radius=50 构建空间图"""
    coor = adata.obsm[spatial_key]
