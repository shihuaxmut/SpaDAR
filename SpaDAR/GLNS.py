...
class GLNSampler_BC(torch.nn.Module):
    def __init__(self, num_centroids, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'),num_clusterings=5):
        super().__init__()
        self.device = device
        self.num_centroids = num_centroids
    ...
    def __I_KNN_Padded(self, adj_knn): 
        ...
        adj_knn_coo = adj_knn.tocoo()


        row = torch.from_numpy(adj_knn_coo.row).long().to(self.device)
        col = torch.from_numpy(adj_knn_coo.col).long().to(self.device)

        I_knn = torch.stack([row, col], dim=1)

        counts = np.array(adj_knn.sum(axis=1)).squeeze()
        neighbor_counts = torch.from_numpy(counts).long().to(self.device)
        k_max = neighbor_counts.max().item()
        I_knn_padded = torch.full((adj_knn.shape[0], k_max), -1, dtype=torch.long, device=self.device)
