...
def clustering(z, n_clust, method="kmeans", num_seed=5,seed=0):
    if num_seed > 1:
        pred_labels = []
        if method == "mclust":
        ...
        if method == "mclust":
            clust_labels = __mclust_R__(z, num_cluster=n_clust, random_seed=3407+seed)
        else:
            kmeans = KMeans(n_clusters=n_clust, max_iter=100, random_state=3407+seed)
            kmeans.fit(z)  # Fit on CPU, but consider moving to GPU if necessary
            clust_labels = kmeans.labels_
